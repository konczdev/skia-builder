...
def patch_device_source(dawn_native: Path) -> bool:
	...
	body = (
		...
		"    // Freed allocations are gated to the PENDING command serial, and an\n"
		"    // idle queue submits nothing that would ever complete it — so force\n"
		"    // an (empty) flush to signal that serial, then release everything\n"
		"    // whose serial has passed: recycled pool heaps go back to the OS.\n"
		"    // Mirrors the D3D11 backend's flush-on-reduce and the Vulkan\n"
		"    // backend's FreeRecycledMemory. A true return means deallocations\n"
		"    // are still waiting on the just-signalled serial; the caller's next\n"
		"    // pass picks them up (the documented ReduceMemoryUsage contract).\n"
		"    // (The flush call goes through ExecutionQueueBase, where it is\n"
		"    // public; the d3d12::Queue override narrows it to private.)\n"
		"    QueueBase* queue = GetQueue();\n"
		"    queue->ForceEventualFlushOfCommands();\n"
		"    if (ConsumedError(ToBackend(queue)->SubmitPendingCommands())) {\n"
		"        return false;\n"
		"    }\n"
		"    if (ConsumedError(queue->CheckPassedSerials())) {\n"
		"        return false;\n"
		"    }\n"
